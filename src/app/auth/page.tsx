  ...
  return (
    <div className="min-h-[100dvh] bg-bg flex flex-col items-center justify-center p-6">
      ...
      <div className="w-full max-w-xs mb-8">
        <p className="label text-tertiary mb-3 text-center">Quick login</p>
        <div className="grid grid-cols-3 gap-2">
          {DEMO_STAFF.map(s => (
            <button
              ...
              disabled={loading}
              className="card p-3 flex flex-col items-center gap-1.5 hover:border-accent/30 active:scale-95 transition-all disabled:opacity-50"
            >
      ...
      <div className="w-full max-w-xs mb-6">
        <p className="label text-tertiary mb-3 text-center">Or enter PIN</p>
          ...
          {[0,1,2,3].map(i => (
            <div
              ...
              className={`w-14 h-14 rounded-2xl border-2 flex items-center justify-center transition-all duration-150 ${
                pin.length > i
                  ? error ? 'border-danger bg-danger/10' : 'border-accent bg-accent/10'
        ...
        {error && (
          <p className="text-danger text-xs text-center mt-2">Wrong PIN — try again</p>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 w-full max-w-xs">
        {DIGITS.map((d, i) => {
          ...
          return (
            <button
              key={i}
              onClick={() => isBack ? handleBackspace() : appendDigit(d)}
              disabled={loading && !isBack}
              className={`h-16 rounded-2xl font-bold text-xl flex items-center justify-center transition-all active:scale-90 disabled:opacity-40 ${
                isBack
                  ? 'bg-hover border border-border text-secondary text-base hover:text-primary'
                  : 'bg-surface border border-border text-primary hover:border-accent/30 hover:bg-accent/5'
              }`}
            >
              {loading && !isBack && d === pin[pin.length - 1]
